...
class LinearMergingStrategyWithRemoval(BaseMergingStrategy):
    ...
    def _merge(self, number_components: int, step: int) -> BaseScoringFunction:
        sf_parameters_to_combine = self._scoring_table.get_sf_components_by_rank(rank=number_components)
    ...
    def run(self) -> Tuple[Model, int]:
        ...
        step_counter = 0
        num_components = len(self._scoring_table.scoring_table[self._scoring_table_enum.SCORES].unique())
        ...
        for sf_component in range(num_components):
            ...
            # extract the user defined merge threshold for the specific helper component
            current_threshold = self._scoring_table.get_sf_components_by_rank(rank=sf_component)[0]\
                [self._special_parameters_enum.SPECIFIC_PARAMETERS][self._special_parameters_enum.MERGING_THRESHOLD]
            agent, step_counter = self.train_agent(agent=agent, optimiser=optimiser, scoring_function=scoring_function,
                                                   ...
                                                   merging_threshold=current_threshold, final_component=final_component)

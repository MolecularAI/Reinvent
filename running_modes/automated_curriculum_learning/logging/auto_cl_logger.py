...
from running_modes.automated_curriculum_learning.logging import LocalAutoCLLoggerCurriculum
...
class AutoCLLogger:
    ...
    def __new__(cls, configuration: GeneralConfigurationEnvelope) -> BaseAutoCLLogger:
        ...
        auto_cl_config = ReinforcementLoggerConfiguration(**configuration.logging)
        
        if auto_cl_config.recipient == logging_mode_enum.LOCAL:
            logger_instance = LocalAutoCLLogger(configuration)

        elif auto_cl_config.recipient == logging_mode_enum.LOCAL_CURRICULUM:
            logger_instance = LocalAutoCLLoggerCurriculum(configuration)

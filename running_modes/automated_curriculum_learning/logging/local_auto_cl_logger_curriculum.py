...
from reinvent_chemistry.logging import add_mols
...
from running_modes.automated_curriculum_learning.logging import ConsoleMessage


class LocalAutoCLLoggerCurriculum(BaseAutoCLLogger):
    ...
    def _tensorboard_report(self, step, smiles, score, score_summary: FinalSummary, agent_likelihood, prior_likelihood,
        ...
        self._summary_writer.add_scalars("Scoring", {"Average score":  np.mean(score)}, step)
        
        for i, log in enumerate(score_summary.profile):
            self._summary_writer.add_scalars("Scoring", {score_summary.profile[i].name: np.mean(score_summary.profile[i].score)},step)
                                                  
        self._summary_writer.add_scalar("Valid SMILES", fraction_valid_smiles(smiles), step)
